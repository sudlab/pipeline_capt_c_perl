...
@follows(mkdir("analysis.dir"), perlDigestGenome)
...
def analyzeInteractions(infile, outfile):
    ...
    # Map the file to be searched for the contigs
    contigs = PARAMS["environment_contigs"]
    
    # Get temporal directory to create contigs file in perl script
    temp_contigs = PARAMS["environment_tempcontigs"]
    ...
    # Full paths for the perl program to not cause errors
    full_contigs_mapping = os.path.join(temp_contigs, (genome+"_sizes.txt"))
    ...
    # Link the contigs file  /shared/sudlab1/General/annotations/hg19_ensembl75/contigs.tsv
    statement = ''' cp %(perl_scripts)s .;
                    rm %(full_contigs_mapping)s;
                    ln -fs %(contigs)s %(full_contigs_mapping)s;
